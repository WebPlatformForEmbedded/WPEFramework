...
namespace Core {
    ...
    template<uint32_t N>
    inline const char *_Err2Str() {
        return _T("");
    };

    #define ERROR_CODE(CODE, VALUE) \
        const uint32_t CODE = VALUE; \
        template<> inline const char *_Err2Str<VALUE>() { return _T(#CODE); }
    ...
    constexpr uint32_t ERROR_LAST = 41;

    template<uint32_t N>
    inline const char* _bogus_ErrorToString(uint32_t code) 
    {
        return (code == N? _Err2Str<N>() : _bogus_ErrorToString<N-1>(code));
    };

    template<>
    inline const char* _bogus_ErrorToString<0ul>(uint32_t code) 
    {
        return (code == 0? _Err2Str<0ul>() : _Err2Str<~0ul>());
    };

    inline const char* ErrorToString(uint32_t code) 
    {
        return _bogus_ErrorToString<ERROR_LAST>(code);
    }
