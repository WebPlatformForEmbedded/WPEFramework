...
private:
    ...
    public:
        ...
        {
            return (reinterpret_cast<EGLNativeWindowType>(_nativeWindow));
        }
...
public:
    ...
    {
        const void* const_pointer = (const_cast<EnumeratedModeSets&>(_platform)[ModeSet::TYPE::CURRENT].UnderlyingHandle());

        void* pointer = const_cast<void*>(const_pointer);

        if(pointer != nullptr) {
            return reinterpret_cast<EGLNativeDisplayType>(pointer);
        } else {
            TRACE(CompositorClient, (_T("The native display (id) might be invalid / unsupported. Using the EGL default display instead!")));
            return reinterpret_cast<EGLNativeDisplayType>(EGL_DEFAULT_DISPLAY);
        }
    ...
    bool ScanOut(const ISurface& surface) override {
        void* window = reinterpret_cast<void*>(surface.Native());

        if(window != nullptr) {
            return _platform[ModeSet::TYPE::CURRENT].ScanOut(reinterpret_cast<const struct gbm_surface*>(window));
        }
...
private:
    ...
    {
        _displaysize = {_platform[ModeSet::TYPE::CURRENT].ScanOutBufferWidth(), _platform[ModeSet::TYPE::CURRENT].ScanOutBufferHeight()};
    ...
    inline void Initialize()
    {
        ...
        if(_platform.Initialize() != true) {
            TRACE(CompositorClient, (_T("Failed to initialize the underlying platform!")));
        }
        else {
            TRACE(CompositorClient, (_T("Succesfully initialized the underlying platform!")));
        }
    ...
    inline void Deinitialize()
    {
        ...
        if(_platform.Deinitialize() != true) {
            TRACE(CompositorClient, (_T("Failed to deinitialize the underlying platform!")));
        }
        else {
            TRACE(CompositorClient, (_T("Succesfully deinitialized the underlying platform!")));
        }
    ...
    void* _virtualinput;
    /* const */ mutable  DisplaySize _displaysize;
    ...
    , _destination({ 0, 0, width, height})
{
    ...
    // By construction _display should exist

    // Mode sets are initialized via Instance -> AddRef and not the Display constructor

    /* DisplaySize */ _display.RetrieveDisplaySize();

    // The scan out buffer should be large enough
    if ((width < _display.DisplaySizeWidth()) || (height < _display.DisplaySizeHeight())) {
        TRACE(CompositorClient, (_T("Requested surface dimensions [%d, %d] might not be honered. Rendering might fail!"), width, height));

        _width < _display.DisplaySizeWidth() ? _display.DisplaySizeWidth() : _width;
        _height < _display.DisplaySizeHeight() ? _display.DisplaySizeHeight() : _height;
    }

    // The underlying display handle can be retrieved this way
    void* handle = reinterpret_cast<void*>(_display.Native());

    // A Native surface that acts as a native window
    const void* pointer = reinterpret_cast<const void*>(ModeSet::CreateRenderTargetFromUnderlyingHandle(reinterpret_cast<struct gbm_device*>(handle), width, height));

    if (pointer != nullptr) {
        _nativeWindow = reinterpret_cast<EGLNativeWindowType>(pointer);
    } else {
        TRACE(CompositorClient, (_T("The native window (handle) might be invalid / unsupported. Expect undefined behavior!")));
        _nativeWindow =  reinterpret_cast<EGLNativeWindowType>(nullptr);
    }
...
Display::SurfaceImplementation::~SurfaceImplementation()
{
    ...
    if (ModeSet::DestroyRenderTargetFromUnderlyingHandle(reinterpret_cast<struct gbm_device*>(_display.Native()), reinterpret_cast<struct gbm_surface*>(_nativeWindow)) != true) {
        TRACE(CompositorClient, (_T("The native window (handle) cannot be destroyed. Leaking resources!")));
    }
}
    ...
    const uint32_t value)
{
    ...
    TRACE(CompositorClient, (_T("Changing display opacity is not (yet) supported!")));
}

uint32_t Display::SurfaceImplementation::Geometry(const Exchange::IComposition::Rectangle& rectangle)
{
    TRACE(CompositorClient, (_T("Changing display geometry is not (yet) supported!")));
...
{
    TRACE(CompositorClient, (_T("Z-order might not correspond to expected layer.")));
    return (_destination);
}

uint32_t Display::SurfaceImplementation::ZOrder(const uint16_t zorder)
{
    TRACE(CompositorClient, (_T("Changing z-order is not (yet) supported!")));
    return (Core::ERROR_NONE);
}
...
Compositor::IDisplay* Compositor::IDisplay::Instance(const string& displayName)
{
    ...
    // At this point the mode sets are initialized via AddRef() -> Initialize()
    // That implies width and heigh are not known beforehand
